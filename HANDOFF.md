...
> 21. `.auto-memory/reference_radio_store_slices.md` — `s.playback.isPlaying` not `s.isPlaying` (slice-confusion trap)
> 22. `.auto-memory/project_deploy_checklist.md` — deploy punch list if that's where we are
>
...
**Post-M18 deploy gate:** the production Supabase needs the `song_id_cache` block from `supabase/schema.sql` applied (idempotent, paste into SQL Editor) before pushing the M18 commit, or the route will fall through to AudD on every tap (cache lookup will silently fail, AudD will succeed, upsert will silently fail) — functionally OK but more expensive than designed.

**Git state:** Working tree clean on `main`. Latest commits:
- `<robots>` — Add robots.txt — replace with actual hash after `git push`
- `<analytics>` — Add Vercel Analytics
- `<m17b>` — HANDOFF: document M17 (reduced-motion caption + font-swap re-measure)
- `<m17>` — Reduced-motion: caption swaps marquee for drag-to-pan (M17 followup)
- `<m17a>` — Marquee: re-measure after web font swap (M17 — defensive belt-and-suspenders, not the actual root cause; kept anyway)
- `24b62ed` — Tighten cabinet vertical spacing (M16)
- `9874129` — Cabinet layout: POWER upper-left, SUGGESTION BOX centered, ON AIR midway, Model No. into dial face
- `1543946` — Suggestion Box: cream contact-email field, directory search w/ autopopulate
- `dee2c74` — Initial commit through M13 + seed-band repair
...
| M18 | NOW PLAYING tap-to-identify song-ID lozenge. Brass plaque (340/420px wide, brass face plate + four corner screws + recessed cream window) centered directly under the dial face. Tap captures ~8s of the live audio output via a new `MediaStreamAudioDestinationNode` (`captureDest`) connected in parallel to the analyser output (so capture is independent of user volume / Doze fade), POSTs it as multipart form-data to a new `POST /api/song-id` route. Route checks new Supabase `song_id_cache` table (60s TTL, station-level, anon-permissive RLS) before forwarding to AudD via FormData. State machine: idle / listening / success / unknown / error — actual error message surfaced in the cream window for diagnosability rather than hidden behind generic "tap to retry". Reduced-motion gate on the listening dot's pulse animation (M17 lesson). Lozenge dims to 0.55 opacity and shows "turn on the radio to identify a song" when `playback.isPlaying` is false. ON AIR lamp re-aligned to `items-end` of its spacer with `mt-6 sm:mt-8 pb-1 sm:pb-2` so the larger NOW PLAYING plate gets breathing room. New env var `AUDD_API_TOKEN` (server-side, no NEXT_PUBLIC prefix). Cache table reads/writes with the existing anon key. New schema block at the bottom of `supabase/schema.sql` (idempotent). One slice-confusion bug caught during iteration — the lozenge initially read `s.isPlaying` (top-level, undefined) instead of `s.playback.isPlaying` (nested), gating the lozenge off forever; saved as a reference memory | `supabase/schema.sql`, `src/app/api/song-id/route.ts`, `src/lib/audio.ts`, `src/components/NowPlayingLozenge.tsx`, `src/components/Console.tsx` |
